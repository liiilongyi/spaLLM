...
def train_spallm_model(adata_rna, adata_protein, embedding, datatype, device):
...
def save_umap_visualization(adata, save_path):
    """Save UMAP visualizations for spaLLM results."""
    ...
    sc.pl.embedding(adata, basis='spatial', color='spaLLM', ax=ax_list[1], title='spaLLM', s=50, show=False)
    plt.tight_layout(w_pad=0.3)
    plt.savefig(save_path, format='jpeg', bbox_inches='tight', dpi=300)
    plt.show()

def save_violin_plot(adata, alpha_key, title, save_path):
    """Save violin plots for embedding weights."""
    ...
    ax = sns.violinplot(data=df, x='label_spaLLM', y='Weight value', hue='Modality', split=True, inner='quart', linewidth=1)
    ax.set_title(title)
...
def main():
    ...
    # Load data
    file_fold = 'D:/lilongyi/spatial dataset/Spatial_CITE_seq_human_tonsil/'
    adata_rna = sc.read_h5ad(file_fold + 'RNA.h5ad')
    adata_protein = sc.read_h5ad(file_fold + 'protein.h5ad')
    ...
    # Train spaLLM model
    output = train_spallm_model(adata_rna, adata_protein, embedding, datatype='10x', device=device)
    ...
    # Visualization
    save_umap_visualization(adata, 'D:/lilongyi/SpatialGlue/dataset/benchmark/Tonsil/umap.jpeg')
    save_violin_plot(adata, 'alpha_att1', 'embedding_spa vs omic', 'D:/lilongyi/SpatialGlue/dataset/benchmark/Tonsil/alpha_att1.jpeg')
    save_violin_plot(adata, 'alpha_att2', 'embedding_fea vs omic', 'D:/lilongyi/SpatialGlue/dataset/benchmark/Tonsil/alpha_att2.jpeg')
